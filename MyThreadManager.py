...
class MyThreadManager:
    ...
    class trade_info:
        thread_id: str = ""
        buy_id: str = ""
        buy_amount: Decimal = Decimal('0.0')
        buy_price: Decimal = Decimal('0.0')
        bought_amount: Decimal = Decimal('0.0')
        buy_remaining_amount: Decimal = Decimal('0.0')
        trade_count: int = 0
        sell_id: str = ""
        sell_price: Decimal = Decimal('0.0')
        sell_amount: Decimal = Decimal('0.0')
        sold_price: Decimal = Decimal('0.0')
        sold_amount: Decimal = Decimal('0.0')
        sell_remaining_amount: Decimal = Decimal('0.0')
        estimated_diff: Decimal = Decimal('0.0')
        isChange: int = 1
        success_time: datetime = datetime(1, 1, 1, second=0)
        trash_coin: Decimal = Decimal('0.0')
    ...
    def add_trade(self, buy_id,buy_amount, buy_price, buy_remaining_amount, sell_price, mode, trade_count):
        ...
        add_trade.buy_amount = self.changeDecimal(buy_amount,8)
        add_trade.buy_price = self.changeDecimal(buy_price,2)
        add_trade.buy_remaining_amount = self.changeDecimal(buy_remaining_amount,8)
        add_trade.sell_price = self.changeDecimal(sell_price,2)
        ...
        with self.lock:
            ...
            if self.active_thread_count > 0:
                for key, trade_info in self.trade_list.items() :
                    ...
                    tot_buy_req_amount += trade_info.sell_remaining_amount
